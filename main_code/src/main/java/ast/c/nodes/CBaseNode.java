...
public abstract class CBaseNode extends AbstractSyntaxNode
{
	...
	{
		final AbstractSyntaxNode copy = this.copy();

		for (final AbstractSyntaxNode child : this.getChildren())
		{
			copy.addChildAndForceParent(child.deepCopy());
		}
	...
	public AbstractSyntaxNode getSuccessorOfType(final Class<? extends CBaseNode> type)
	{
		for (final AbstractSyntaxNode child : this.getChildren())
		{
			final AbstractSyntaxNode successorOfType = this.getSuccessorOfType(child, type);

			if (successorOfType != null)
			{
				return successorOfType;
			}
		}
	...
	private AbstractSyntaxNode getSuccessorOfType(final AbstractSyntaxNode currentNode,
												  final Class<? extends CBaseNode> type)
	{
		if (type.isInstance(currentNode))
		{
			//We found a node of expected type
			return currentNode;
		}
